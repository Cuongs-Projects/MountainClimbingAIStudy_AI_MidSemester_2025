...
class URDFLink:
    ...
    def to_link_element(self, adom):
        ...
        if shape_val <= 0.33:
            ...
            # 'size' expects full dimensions (x, y, z)
            # If your link_radius is used for x/y half-extents, then full x/y is 2*link_radius
            # If your link_length is used for z full-extent, then it's just link_length
        ...
        #to now change shape's collision boundary#Touched this
        if shape_val <= 0.33:
        ...
        #to now change shape's mass#Touched this
        mass = 0.1
        if shape_val <= 0.33:
            ...
            mass = (4/3) * np.pi * (self.link_radius**3)
