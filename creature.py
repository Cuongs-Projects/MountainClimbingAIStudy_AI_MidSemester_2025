...
class Motor:
    ...
    def get_output(self, touch = False):
        #if touching, generate a constant "push away" force.
        if touch == True: #Touched this
            return self.amp * -1.0
...
class Creature:
    def __init__(self, gene_count):
        ...
        self.closest_position = None #Touched this
    ...
    def get_motors(self):
        self.get_expanded_links()
        if self.motors == None:
            #a motor for each JOINT.
            #creature has len(self.exp_links) limbs.
            #creature has len(self.exp_links) - 1 joints (the base link has no parent joint).
            #motors list should align with the joint indices (0 to N-1).
            motors = []
            #start the loop at 1 to skip the base link (spine1), which has no motor.
    ...
    #Touched this
    def distance_to_peak(self,height = 6): #6 because the mouintain z is istantiated at -1
        #if no closest_position = creature cant find stable place to stay in the arena / too big
