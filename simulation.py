...
class Simulation: 
    ...
    #moved the functions to generate the environment from cw-envt.py into here#Touched this
    ...
    def run_creature(self, cr, iterations=2400,current_iter = 0):
        ...
        max_height = guassian_pyramid_height + 3.0 # e.g., 3 meters above the peak is too high#Touched this
        min_height = -1.5 # e.g., 1.5 meters below the floor is out of bounds#Touched this
        
        # This will penalize going outside the square arena or flying too high/falling too low#Touched this
        ...
        num_joints = p.getNumJoints(cid, physicsClientId=pid) #Touched this
        ...
        p.resetBasePositionAndOrientation(cid, [7, 7, 8], [0, 0, 0, 1], physicsClientId=pid)
        ...
        if touched_ground == True:
            ...
            for step in range(iterations):
                ...
                #for reactive motors
                #dictionary will store which limbs are touching something.
                #keys will be link indices (0, 1, 2, ...), value will be True/False.
                touch_data = {} #Touched this
                contacts = p.getContactPoints(bodyA=cid, physicsClientId=pid)
                for contact in contacts:
                ...
                pos, orn = p.getBasePositionAndOrientation(cid, physicsClientId=pid)#Touched this
                cr.update_position(pos)

                #boundary check#Touched this
    ...
    def update_motors(self, cid, cr,touch_data):
        motors = cr.get_motors()
        #loop through all the JOINTS of the creature
        for jid in range(p.getNumJoints(cid,physicsClientId=self.physicsClientId)):#Touched this

            child_link_index = jid 

            #check if this specific limb has touch data.
            # .get(key, False) is a safe way to check a dictionary.
            #returns the value for the key if it exists, otherwise it returns False.
            is_touching = touch_data.get(child_link_index, False)
            
            m = motors[jid]

            #call the motor's get_output method, now with the sensory information.
            vel = m.get_output(touch=is_touching)

            p.setJointMotorControl2(cid, jid, 
                    controlMode=p.VELOCITY_CONTROL, 
                    targetVelocity=vel, 
                    force = 10, #default 5, final 10
